...
$query=mysqli_query($con,"Select Password FROM tbladmin where MobileNumber='$adminid'");
$row=mysqli_fetch_array($query); 
if($row['Password']==$password){  
    $money=$_POST['money']; 
    $query=mysqli_query($con,"insert into deposit(Account,BankName,Reference,Amount) values('$account','$bankname','$refid','$money')");     
    echo "<script>alert('Deposited Amount is done successfully. Wait por Confermation');</script>";   
...
<li class="breadcrumb-item active" aria-current="page"><?php echo $row['AdminName'];?></li>
            ...
            <div class="container">
               ...
               <div class="hk-pg-header">
                    <h4 class="hk-pg-title"><span class="pg-title-icon"><span class="feather-icon"><i data-feather="external-link"></i></span></span>Deposit</h4>
                </div>
                <!-- /Title -->
...
<select class="form-control"  id="select-state" placeholder="Select a Bank..." name="BankName" required>
    <option class="" style="color:rgb(222,212,212)" >Select a Bank...</option>
    <?php 
    $sql = "SELECT * from  bankname";
...
<input type="number" class="form-control" id="validationCustom03" value="<?php echo $_SESSION['account'];?>" name="AcNo">
</div>
</div>
