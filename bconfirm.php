...
if (strlen($_SESSION['aid']==0)) {
  header('location:logout.php');
        ...
        if($row['Pin']==$pin){ 
            ...
            $query=mysqli_query($con,"INSERT into cashouttbl (UserID,BankName,Account,PBalance,Cashout,NewBalance,Comment) values('$adminid','$bank','$account','$correntBal','$Money','$newBal','2')");
            unset ($_SESSION['money']);
            unset ($_SESSION['BankName']);
            echo "<script>alert('Bank Transfer Successfull & new Balance : $newBal.');</script>"; 
            ?>
            <script>window.location.href='dashboard.php'</script>";<?php
            }else{
                echo "<script>alert('Cash Out Failed');</script>"; ?>
                 <script>window.location.href='banktrans.php'</script><?php
            }
        }
        else{
