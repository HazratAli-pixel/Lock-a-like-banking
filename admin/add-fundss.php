...
// Code for deletion       
    if(isset($_GET['add'])){  
        ...
        $time = $_GET['time'];
        $query1=mysqli_query($con,"select * from deposit where Account='$cmpid' AND Inputdate='$time'");
        ...
        $query4=mysqli_query($con,"update deposit set Status='1' where Account='$cmpid' AND Inputdate='$time' "); 

    
    echo "<script>alert('Funds are Added.');</script>";   
    echo "<script>window.location.href='add-fundss.php'</script>";
    }
    if(isset($_GET['del'])){  
        $ref=$_SESSION['admin'];  
                                    ...
                                    <div style="overflow-x:scroll;">
                                        <table style="min-width: 600px;"  id="datable_1" class=" table table-hover w-100 display pb-30">
                                            ...
                                            <tbody>
                                                ...
                                                $ref=$_SESSION['admin'];
                                                $query = mysqli_query($con,"SELECT * FROM deposit order by Status");
                                                $cnt=1;
                                            ...
                                            <?php }else { ?> <button type="button" class="btn btn-success">Success</button> <?php }
                                                ...
                                                <?php if ($row['Status']!=1){ ?>
                                                <a href="add-fundss.php?add=<?php echo base64_encode($row['Account'].$rno);?>&time=<?php echo $row['Inputdate'];?>"  class="mr-25" data-toggle="tooltip" data-original-title="Add" onclick="return confirm('Do you really want to add?');"> <i class="icon-plus txt-danger"></i> </a> 
                                                <a href="add-fundss.php?del=<?php echo base64_encode($row['Account'].$rno);?>&time=<?php echo $row['Inputdate'];?>" data-toggle="tooltip" data-original-title="Delete" onclick="return confirm('Do you really want to Delete?');"> <i class="icon-trash txt-danger"></i> </a><?php } ?>
                                                </td>
                                                </tr>
                                                <?php 
                                                $cnt++;
